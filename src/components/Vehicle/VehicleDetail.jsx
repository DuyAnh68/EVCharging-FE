...
function VehicleDetail() {
  ...
  if (!vehicle) {
    return (
      <div className="p-8">
        <div className="text-2xl font-bold text-red-600">Xe không tồn tại.</div>
        <Link to="/vehicle-list" className="text-[#009951] underline mt-4 block">
  ...
  return (
    <div className="bg-[#F6F9EE] min-h-screen flex flex-col items-center px-6 py-8">
      <div className="max-w-xl w-full bg-white rounded-lg shadow p-6">
        ...
        />
        <div className="font-bold text-2xl text-[#14AE5C] mb-2">{vehicle.name}</div>
        <div className="mb-1 text-gray-700">Biển số: <span className="font-medium">{vehicle.licensePlate}</span></div>
