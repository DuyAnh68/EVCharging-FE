...
const CompanyVehicleListGrid = () => {
  const { getCompanyVehicle, loading, error } = useCompany();
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {vehicles?.map((v) => (
          <CompanyVehicleCard key={v.id} data={v} onDeleted={handleDeleted} />
        ))}
      </div>
    </div>
  );
...
export default CompanyVehicleListGrid;
